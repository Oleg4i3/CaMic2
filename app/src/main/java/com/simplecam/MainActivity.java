...
public class MainActivity extends Activity implements SurfaceHolder.Callback {
    ...
    private static class EncodedFrame {
        ...
        EncodedFrame(ByteBuffer src, MediaCodec.BufferInfo bi) {
            ...
            src.position(bi.offset);
            src.get(data, 0, bi.size);
    ...
    private volatile boolean mDigitalStabEnabled = false;
    private ImageReader mStabImageReader;
    private Surface mStabSurface;
    private HandlerThread mStabThread;
    private Handler mStabHandler;
    private byte[] mPrevYPlane;
    private int mPrevWidth, mPrevHeight;
    private final Object mStabLock = new Object();
    private volatile float mStabOffsetX = 0f, mStabOffsetY = 0f;
    private volatile float mStabTargetX = 0f, mStabTargetY = 0f;
    private final float STAB_SMOOTH = 0.3f;
    private static final int STAB_ANALYSIS_WIDTH = 320;
    private static final int STAB_ANALYSIS_HEIGHT = 180;
    ...
    protected void onDestroy() {
        ...
        finalizeMuxer();
        if (mVidEnc != null) {
            try { mVidEnc.stop(); mVidEnc.release(); } catch (Exception e) {}
            mVidEnc = null;
        }
        if (mEncSurface != null) {
            try { mEncSurface.release(); } catch (Exception e) {}
            mEncSurface = null;
        }
    ...
    private View buildLayout() {
        ...
        FrameLayout.LayoutParams vuVertLP = new FrameLayout.LayoutParams(
        ...
        mSeekEv.setLayoutParams(new LinearLayout.LayoutParams(
        ...
        sbPB.setMax(4);
        sbPB.setProgress(0);
        sbPB.setLayoutParams(new LinearLayout.LayoutParams(
    ...
    private void startPreview() {
        ...
        try {
            ...
            if (mDigitalStabEnabled) {
                setupStabilizationSurface();
                if (mStabSurface != null) targets.add(mStabSurface);
            } else {
                releaseStabilizationSurface();
            }
    ...
    private void buildAndSendRequest() {
        ...
        try {
            ...
            if (mDigitalStabEnabled && mStabSurface != null)
                rb.addTarget(mStabSurface);
            ...
            if (mSensorRect != null) {
                ...
                float stabMargin = 0f;
                if (mDigitalStabEnabled && mZoomLevel > 1.01f) {
                    float zoomFactor = (mZoomLevel - 1f) / (mMaxZoom - 1f);
                    stabMargin = 0.05f + 0.15f * zoomFactor;
                    effectiveZoom = mZoomLevel / (1f - stabMargin);
                ...
                if (mDigitalStabEnabled && mZoomLevel > 1.01f) {
                    float offX = mStabOffsetX, offY = mStabOffsetY;
                    float maxShiftX = (cropW * stabMargin) / 2f;
                    float maxShiftY = (cropH * stabMargin) / 2f;
                    int shiftX = Math.round(offX * maxShiftX);
                    int shiftY = Math.round(offY * maxShiftY);
                    cropX += shiftX;
                    cropY += shiftY;
                    cropX = Math.max(mSensorRect.left, Math.min(cropX, mSensorRect.right - cropW));
                    cropY = Math.max(mSensorRect.top, Math.min(cropY, mSensorRect.bottom - cropH));
                }
    ...
    private void setupStabilizationSurface() {
        if (mStabImageReader != null) return;
        mStabThread = new HandlerThread("StabAnalyzer");
        mStabThread.start();
        mStabHandler = new Handler(mStabThread.getLooper());

        mStabImageReader = ImageReader.newInstance(
                STAB_ANALYSIS_WIDTH, STAB_ANALYSIS_HEIGHT, ImageFormat.YUV_420_888, 2);
        mStabSurface = mStabImageReader.getSurface();
        mStabImageReader.setOnImageAvailableListener(reader -> {
            Image image = reader.acquireLatestImage();
            if (image != null) mStabHandler.post(() -> processStabilizationFrame(image));
        }, mStabHandler);
    }

    private void releaseStabilizationSurface() {
        if (mStabImageReader != null) {
            mStabImageReader.close();
            mStabImageReader = null;
            mStabSurface = null;
        }
        if (mStabThread != null) {
            mStabThread.quitSafely();
            mStabThread = null;
            mStabHandler = null;
        }
        mPrevYPlane = null;
    ...
    private void processStabilizationFrame(Image image) {
        try {
            ...
            if (yPixelStride == 1) {
                ...
                for (int row = 0; row < height; row++) {
                    for (int col = 0; col < width; col++) {
                        yPlane[row * width + col] = yBuffer.get(row * yRowStride + col * yPixelStride);
                    }
                }
            }

            synchronized (mStabLock) {
                if (mPrevYPlane != null && mPrevWidth == width && mPrevHeight == height) {
                    int searchRange = 16;
                    ...
                    for (int dy = -searchRange; dy <= searchRange; dy += 2) {
                        for (int dx = -searchRange; dx <= searchRange; dx += 2) {
                            ...
                            for (int y = yStart; y < yEnd; y += 4) {
                                int idx1 = y * width + xStart;
                                int idx2 = (y + dy) * width + (xStart + dx);
                                for (int x = xStart; x < xEnd; x += 4) {
                                    int v1 = mPrevYPlane[idx1] & 0xFF;
                                    int v2 = yPlane[idx2] & 0xFF;
                                    corr += v1 * v2;
                                    count++;
                                    idx1 += 4;
                                    idx2 += 4;
                    ...
                    float normDx = bestDx / (float) (width / 2);
                    float normDy = bestDy / (float) (height / 2);
                    normDx = Math.max(-1f, Math.min(1f, normDx));
                    normDy = Math.max(-1f, Math.min(1f, normDy));

                    mStabTargetX = mStabTargetX * (1f - STAB_SMOOTH) + normDx * STAB_SMOOTH;
                    mStabTargetY = mStabTargetY * (1f - STAB_SMOOTH) + normDy * STAB_SMOOTH;
                    mStabOffsetX = mStabTargetX;
                    mStabOffsetY = mStabTargetY;
    ...
    private synchronized void ensureEncoders() {
        if (mVidEnc == null || mEncSurface == null || !mEncSurface.isValid()) {
            try {
                ...
                status("VidEnc err: " + e.getMessage());
                return;
            }
        }
        if (!mVidLoopRunning) {
    ...
    private void doStart() {
        try {
            ...
            runOnUiThread(() -> {
                ...
                mBtnPause.setVisibility(View.VISIBLE); mBtnPause.setText("⏸"); mBtnPause.setBackground(mBtnBgPause);
            ...
            runOnUiThread(() -> {
                ...
                mBtnPause.setVisibility(View.GONE); status("Error: " + e.getMessage());
            });
        }
    }
    ...
    private void finalizeMuxer() {
        ...
        runOnUiThread(() -> {
            mBtn.setText("⏺ REC"); mBtn.setBackground(mBtnBgIdle); mBtn.setEnabled(true);
            mBtnPause.setVisibility(View.GONE); mPaused = false; status("Saved");
        });
    }
    ...
    private static class AudioSrcItem {
        ...
        AudioSrcItem(String n, int s, AudioDeviceInfo d) { name = n; audioSource = s; device = d; }
    }

    // ────────────────────────────────────────────────────────────────────────────────
    // Вспомогательные View-классы
    // ────────────────────────────────────────────────────────────────────────────────
    ...
    static class VerticalSeekBar extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            canvas.drawRoundRect(new RectF(trkX1, trkT, trkX2, trkB), trackW/2f, trackW/2f, mTrackPaint);
            canvas.drawRoundRect(new RectF(trkX1, thumbY, trkX2, trkB), trackW/2f, trackW/2f, mFillPaint);
            ...
            shadow.setColor(0x66000000); shadow.setStyle(Paint.Style.FILL);
            canvas.drawRoundRect(new RectF(fader.left+2, fader.top+3, fader.right+2, fader.bottom+3), dp(4), dp(4), shadow);
    ...
    static class VuMeterView extends View {
        ...
        void setPeak(float peak) {
            ...
            if (db >= mPeakDb || System.currentTimeMillis() > mPeakHoldUntil) {
                mPeakDb = db; mPeakHoldUntil = System.currentTimeMillis() + PEAK_HOLD_MS;
            }
        }
    ...
    static class ZoomLeverView extends View {
        ...
        private void recalc() {
            ...
            trkT = lblH; trkB = h - lblH; trkH = trkB - trkT;
            trkW = dp(16); mid = (trkT + trkB) / 2f; cx = w / 2f;
        }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            canvas.drawRoundRect(new RectF(cx - thumbW/2f, thumbCY - thumbH/2f, cx + thumbW/2f, thumbCY + thumbH/2f), dp(6), dp(6), mThumbPaint);
        ...
        private final Runnable mSpring = new Runnable() {
            ...
            public void run() {
                ...
                else { mPos = 0f; if (mListener != null) mListener.onLever(0f); invalidate(); }
            }
        };
    ...
    static class FocusDrumView extends View {
        interface OnFocusChangeListener { void onFocusChanged(float value); }
        interface OnDrumScrollListener { void onScrollStart(); void onScrollStop(); }
        ...
        void setOnFocusChangeListener(OnFocusChangeListener l) { mListener = l; }
        void setOnDrumScrollListener(OnDrumScrollListener l) { mScrollListener = l; }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            for (int i = -1; i <= totalRisks; i++) {
                ...
                mRiskPaint.setColor(0xFFFFFFFF); mRiskPaint.setAlpha(alpha);
        ...
        public boolean onTouchEvent(MotionEvent e) {
            switch (e.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE: {
                    ...
                    if (newVal != mValue) { mValue = newVal; if (mListener != null) mListener.onFocusChanged(mValue); }
                    invalidate();
                    return true;
                }
                case MotionEvent.ACTION_UP:
    ...
    static class OscilloscopeView extends View {
        ...
        void pushSamples(short[] buf, int len, int channels) {
            synchronized (mLock) {
                for (int i = 0; i < len; i += channels) {
                    ...
                    if (channels == 2 && i + 1 < len) mono = (mono + buf[i+1] / 32768f) * 0.5f;
                ...
                if (trigPos < 0) trigPos = (mWritePos - DISP_SAMPLES + BUF_SIZE) % BUF_SIZE;
                for (int k = 0; k < DISP_SAMPLES; k++) mFrame[k] = mRingBuf[(trigPos + k) % BUF_SIZE];
                mNewFrame = true;
            }
            postInvalidate();
        }
        ...
        protected void onDraw(Canvas canvas) {
            final float w = getWidth(), h = getHeight();
            if (w == 0 || h == 0) return;
            for (int gx = 1; gx < 4; gx++) canvas.drawLine(w * gx / 4f, 0, w * gx / 4f, h, mGridPaint);
            for (int gy = 1; gy < 4; gy++) canvas.drawLine(0, h * gy / 4f, w, h * gy / 4f, mGridPaint);
            Paint zeroPaint = new Paint(mGridPaint);
            zeroPaint.setColor(0x55FFFFFF); zeroPaint.setStrokeWidth(1.4f);
            canvas.drawLine(0, h/2f, w, h/2f, zeroPaint);
            Paint trigPaint = new Paint();
            trigPaint.setColor(0x88FFFF00); trigPaint.setStrokeWidth(1f);
            ...
            float[] frame;
            synchronized (mLock) { frame = mFrame.clone(); }
    ...
    static class EnvelopeView extends View {
        ...
        private int mWritePos = 0, mFilled = 0;
        ...
        void pushSamples(short[] buf, int len, int channels) {
            synchronized (mLock) {
                for (int i = 0; i < len; i += channels) {
                    ...
                    if (channels == 2 && i + 1 < len) s = Math.max(s, Math.abs(buf[i+1] / 32768f));
        ...
        protected void onDraw(Canvas canvas) {
            ...
            float[] snap; int filled, writePos;
            synchronized (mLock) { snap = mEnv.clone(); filled = mFilled; writePos = mWritePos; }
            if (filled < 2) { canvas.drawText("ENV  10s", 4, h - 3f, mLabelPaint); return; }
            ...
            Paint curPaint = new Paint(); curPaint.setColor(0x66FFFFFF); curPaint.setStrokeWidth(1f);
    ...
    static class SpectrumView extends View {
        ...
        SpectrumView(Context c) {
            super(c);
            for (int i = 0; i < FFT_SIZE; i++) mWindow[i] = 0.5f * (1f - (float)Math.cos(2.0 * Math.PI * i / (FFT_SIZE - 1)));
        ...
        private void computeFFT() {
            for (int i = 0; i < FFT_SIZE; i++) { mFftRe[i] = mAccBuf[i] * mWindow[i]; mFftIm[i] = 0f; }
            int n = FFT_SIZE;
            ...
            for (int len = 2; len <= n; len <<= 1) {
                ...
                for (int i = 0; i < n; i += len) {
                    ...
                    for (int k = 0; k < len / 2; k++) {
                        ...
                        mFftRe[i + k] = uRe + vRe; mFftIm[i + k] = uIm + vIm;
                        mFftRe[i + k + len/2] = uRe - vRe; mFftIm[i + k + len/2] = uIm - vIm;
        ...
        protected void onDraw(Canvas canvas) {
            ...
            gridPaint.setColor(0x44FFFFFF); gridPaint.setStrokeWidth(0.8f);
            ...
            float[] smooth, peaks;
            synchronized (mLock) { smooth = mSmooth.clone(); peaks = mPeaks.clone(); }

            float logFMin = (float)Math.log10(20f);
