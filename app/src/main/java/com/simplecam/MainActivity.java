...
public class MainActivity extends Activity implements SurfaceHolder.Callback {
    ...
    private View buildLayout() {
        ...
        mSeekEisDrift.setMax(99);
        mSeekEisDrift.setProgress(29); // progress+1 → 0.030
        ...
        eisRow.setOrientation(LinearLayout.HORIZONTAL);
        eisRow.setGravity(Gravity.CENTER_VERTICAL);
        eisRow.addView(smallLabel("Drft:"));
    ...
    private void videoPreviewLoop() {
        ...
        while (mVidLoopRunning) {
            ...
            if (out == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                synchronized (mVidRingLock) {
                    mVidOutFmt = enc.getOutputFormat();
                }
                continue;
            }
            if (out < 0) continue;
            ...
            try {
                ...
                if (mode == 0) {
                    ...
                    synchronized (mVidRingLock) {
                        ...
                        for (EncodedFrame rf : mVidRing) {
                            ...
                            synchronized (mMuxLock) {
                                if (mMuxReady) mMuxer.writeSampleData(mVidTrack, rb, bi);
                            }
                        }
                        mVidRing.clear();
                    }
                    ...
                    synchronized (mMuxLock) {
                        if (mMuxReady) mMuxer.writeSampleData(mVidTrack, data, n);
                    }
                    mVidWriteMode = 2; // LIVE
                } else {
                    // ── LIVE: прямо в мюксер ──
    ...
    private void startMonitor() {
        ...
        try {
            ...
            status("AudEnc err: " + e.getMessage());
            mAudEnc = null;
        }
        ...
        while (mAudRunning) {
            ...
            for (int i = 0; i < r; i++) {
                ...
                if (sc) {
                    ...
                    float ab = Math.abs(s);
                    if (ab > T)
                        s = Math.signum(s) * (T + knee * (float) Math.tanh((ab - T) / knee));
                }
    ...
    private void drainAudioEncoder(MediaCodec enc) {
        ...
        while (true) {
            ...
            if (out == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                synchronized (mAudRingLock) {
                    mAudOutFmt = enc.getOutputFormat();
                }
                continue;
            }
            if (out < 0) break;
                ...
                if (mode == 0) {
                    ...
                    synchronized (mAudRingLock) {
                        ...
                        for (EncodedFrame rf : mAudRing) {
                            ...
                            bi.set(0, rf.data.length, rf.pts - mMuxBasePts, rf.flags);
                            synchronized (mMuxLock) {
                                if (mMuxReady) mMuxer.writeSampleData(mAudTrack, rb, bi);
                            }
                        }
                        mAudRing.clear();
                    }
                    ...
                    n.set(info.offset, info.size, info.presentationTimeUs - mMuxBasePts, info.flags);
                    synchronized (mMuxLock) {
                        if (mMuxReady) mMuxer.writeSampleData(mAudTrack, data, n);
                    }
        ...
        try {
            ...
            if (mVidOutFmt == null || mAudOutFmt == null) {
                runOnUiThread(() -> {
                    ...
                    status("Encoder not ready — retry");
                });
                return;
            }
    ...
    private void doStop() {
        ...
        mVidWriteMode = 0;
        mAudWriteMode = 0; // обратно в RING
        finalizeMuxer();
    }
    ...
    static class VerticalSeekBar extends View {
        ...
        private float faderH(float w) {
            return Math.round(w * 0.7f) + dp(20);
        }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            z.setColor(0x88FFFFFF);
            z.setStrokeWidth(1.5f);
            ...
            cLine.setColor(0xFFDDCC00);
            cLine.setStrokeWidth(1.5f);
    ...
    // Полностью соответствует логике HTML-версии с учётом ориентации камеры.
    // ═══════════════════════════════════════════════════════════════════════════
    private class EisEngine {

        // Параметры анализа (точно как в HTML)
        static final int ANAL_W = 640, ANAL_H = 360;
        static final int TMPL_W = 150, TMPL_H = 150;
        static final float IDEAL_X = (ANAL_W - TMPL_W) / 2f;
        static final float IDEAL_Y = (ANAL_H - TMPL_H) / 2f;
        ...
        private int mProgPass1, mProgPass2;
        private int mU1_stm;
        ...
        // Пиксельный буфер для OpenCV (RGBA)
        private final byte[] mPixBuf = new byte[ANAL_W * ANAL_H * 4];
        ...
        private final Surface mOutEnc;   // может быть null
        ...
        // Pass1: OES → FBO (без STMatrix, сохраняем ориентацию сенсора)
        ...
        // Pass2: FBO → выход со стабилизацией и зумом (учёт ориентации)
        ...
        void stop() {
            ...
            // Ресурсы освободятся в releaseGl()
        }
        ...
        private void glLoop() {
            ...
            while (mRunning) {
                ...
                try {
                    ...
                    renderPass1();                         // OES → FBO (без поворота)
                    analyzeFrame();                        // OpenCV
        ...
        private void initGlObjects() {
            ...
            android.opengl.GLES20.glTexImage2D(android.opengl.GLES20.GL_TEXTURE_2D, 0,
                android.opengl.GLES20.GL_RGBA, ANAL_W, ANAL_H, 0,
            ...
            // Шейдеры
            mProgPass1 = buildProg(VERT, FRAG1);
            mProgPass2 = buildProg(VERT, FRAG2);
            mU1_stm     = android.opengl.GLES20.glGetUniformLocation(mProgPass1, "uSTM");
            mU2_dx      = android.opengl.GLES20.glGetUniformLocation(mProgPass2, "uDx");
            mU2_dy      = android.opengl.GLES20.glGetUniformLocation(mProgPass2, "uDy");
            mU2_invZoom = android.opengl.GLES20.glGetUniformLocation(mProgPass2, "uIZ");
        ...
        private void renderPass1() {
            android.opengl.EGL14.eglMakeCurrent(mEglDisp, mEglPrev, mEglPrev, mEglCtx);
            android.opengl.GLES20.glBindFramebuffer(android.opengl.GLES20.GL_FRAMEBUFFER, mFboId);
            android.opengl.GLES20.glViewport(0, 0, ANAL_W, ANAL_H);
            android.opengl.GLES20.glUseProgram(mProgPass1);
            android.opengl.GLES20.glActiveTexture(android.opengl.GLES20.GL_TEXTURE0);
            android.opengl.GLES20.glBindTexture(android.opengl.GLES11Ext.GL_TEXTURE_EXTERNAL_OES, mOesTex);
            android.opengl.GLES20.glUniform1i(
                android.opengl.GLES20.glGetUniformLocation(mProgPass1, "uTex"), 0);
            drawQuad(mProgPass1);
            android.opengl.GLES20.glBindFramebuffer(android.opengl.GLES20.GL_FRAMEBUFFER, 0);
        }
        ...
        private void renderToSurface(android.opengl.EGLSurface surf, long frameNs, boolean isEnc) {
            ...
            android.opengl.GLES20.glUseProgram(mProgPass2);
            android.opengl.GLES20.glActiveTexture(android.opengl.GLES20.GL_TEXTURE0);
            android.opengl.GLES20.glBindTexture(android.opengl.GLES20.GL_TEXTURE_2D, mFboTex);
            android.opengl.GLES20.glUniform1i(
                android.opengl.GLES20.glGetUniformLocation(mProgPass2, "uTex"), 0);
        ...
        private void analyzeFrame() {
            // Читаем RGBA пиксели из FBO (без STMatrix, ориентация нативная)
            android.opengl.EGL14.eglMakeCurrent(mEglDisp, mEglPrev, mEglPrev, mEglCtx);
            android.opengl.GLES20.glBindFramebuffer(android.opengl.GLES20.GL_FRAMEBUFFER, mFboId);
            java.nio.ByteBuffer bb = java.nio.ByteBuffer.wrap(mPixBuf);
            android.opengl.GLES20.glReadPixels(0, 0, ANAL_W, ANAL_H,
                android.opengl.GLES20.GL_RGBA, android.opengl.GLES20.GL_UNSIGNED_BYTE, bb);
            android.opengl.GLES20.glBindFramebuffer(android.opengl.GLES20.GL_FRAMEBUFFER, 0);

            org.opencv.core.Mat rgba = new org.opencv.core.Mat(ANAL_H, ANAL_W,
            ...
            // Учитываем возможный поворот камеры: если mSensorOrientation = 90 или 270,
            // нужно повернуть координаты анализа (потому что FBO содержит изображение
            // в нативной ориентации сенсора). Для простоты будем считать, что
            // большинство задних камер имеют ориентацию 90°.
            // Однако для корректной работы в общем случае нужно адаптировать размеры.
            // Здесь предполагаем, что изображение в FBO соответствует ANAL_W x ANAL_H,
            // а физический поворот компенсируется на этапе рендера в Pass2.
            // Логика стабилизации остаётся точно как в HTML.
            ...
            int roiW = (int)Math.min(ANAL_W - roiX, TMPL_W + SEARCH_RADIUS * 2);
            int roiH = (int)Math.min(ANAL_H - roiY, TMPL_H + SEARCH_RADIUS * 2);
            ...
            boolean onEdge = matchX < EDGE_MARGIN || matchX + TMPL_W > ANAL_W - EDGE_MARGIN
                          || matchY < EDGE_MARGIN || matchY + TMPL_H > ANAL_H - EDGE_MARGIN;
            ...
            // Нормализация для шейдера.
            // В шейдере u,v ∈ [0,1], dx,dy — в пикселях ANALYSIS.
            // Мы хотим сдвинуть изображение в противоположную сторону движения.
            // В HTML: u = (1-1/zoom)/2 + tx/zoom + uDx
            // где uDx = dx / W (W=ANAL_W). Знак "+" даёт компенсацию движения.
            // Здесь GL-координаты: (0,0) — левый нижний угол, (1,1) — правый верхний.
            // После glReadPixels строка 0 — низ изображения, поэтому dy нужно инвертировать.
            mOutDxNorm =  dx / ANAL_W;
            mOutDyNorm = -dy / ANAL_H;
    ...
    class EisDebugView extends View {
        ...
        @Override
        protected void onDraw(Canvas canvas) {
            final float sw = getWidth()  / (float) EisEngine.ANAL_W;
            final float sh = getHeight() / (float) EisEngine.ANAL_H;
            final float ix = EisEngine.IDEAL_X, iy = EisEngine.IDEAL_Y;
            final float tw = EisEngine.TMPL_W * sw, th = EisEngine.TMPL_H * sh;
